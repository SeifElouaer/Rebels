            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
                ...
                <div className="grid grid-cols-3 gap-4">
                    ...
                    <div className="bg-white/10 rounded-lg px-4 py-2 text-center">
                        ...
                        <p className="text-xl font-bold">{decision.riskTier}</p>
                    </div>
                    <div className="bg-white/10 rounded-lg px-4 py-2 text-center">
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-slate-400">Score: {twin.creditScore} | Income: ${twin.income.toLocaleString()}</p>
                                </div>
                            </div>
...
function AnomalyRadar({ twins, anomalyScore, flags }) {
    ...
    React.useEffect(() => {
        ...
        const timer = setTimeout(() => {
            document.getElementById('maxSimBar').style.width = `${maxSim * 100}%`;
            document.getElementById('densityBar').style.width = `${avgSim * 100}%`;
            document.getElementById('decayBar').style.width = `${Math.min(100, decayRate * 200)}%`;
            document.getElementById('anomalyBar').style.width = `${anomalyScore * 100}%`;
        }, 100);
    ...
    React.useEffect(() => {
        ...
        outcomeChartInstance.current = new Chart(ctx1, {
            ...
            data: {
                ...
                datasets: [{
                    data: [decision.repaidCount, decision.defaultCount],
        ...
        twins.forEach(t => {
            if (t.daysLate === 0) lateBuckets['0']++;
            else if (t.daysLate <= 15) lateBuckets['1-15']++;
            else if (t.daysLate <= 30) lateBuckets['16-30']++;
            else if (t.daysLate <= 60) lateBuckets['31-60']++;
            else lateBuckets['60+']++;
        });
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-slate-800 rounded-xl p-6 card-glow">
                ...
                <div className="mt-4 grid grid-cols-2 gap-4 text-center">
                    <div className="bg-slate-700/50 rounded-lg p-3">
                        <p className="text-2xl font-bold text-green-400">{((1 - decision.defaultRate) * 100).toFixed(0)}%</p>
                        <p className="text-xs text-slate-400">Repaid Rate</p>
                    </div>
                    <div className="bg-slate-700/50 rounded-lg p-3">
                        <p className="text-2xl font-bold text-red-400">{(decision.defaultRate * 100).toFixed(0)}%</p>
            ...
            <div className="bg-slate-800 rounded-xl p-6 card-glow">
                ...
                <div className="mt-4 text-center">
                    ...
                    <p className="text-2xl font-bold text-blue-400">{decision.avgDaysLate.toFixed(1)} days</p>
                </div>
            </div>
        </div>
    );
            ...
            <div className="space-y-4">
                <div className="bg-slate-700/50 rounded-lg p-4">
                    ...
                    <ul className="mt-2 space-y-1 text-slate-300">
                        <li>• <strong>{decision.repaidCount}</strong> borrowers repaid successfully ({((1 - decision.defaultRate) * 100).toFixed(1)}%)</li>
                        <li>• <strong>{decision.defaultCount}</strong> borrowers defaulted ({(decision.defaultRate * 100).toFixed(1)}%)</li>
                        <li>• Average days late in cohort: <strong>{decision.avgDaysLate.toFixed(1)} days</strong></li>
                ...
                <div className="bg-slate-700/50 rounded-lg p-4">
                    ...
                    <ul className="mt-2 space-y-1 text-slate-300">
                        ...
                        <li>• This borrower had credit score {twins[0].creditScore}, income ${twins[0].income.toLocaleString()}, and {twins[0].outcome.toLowerCase()}</li>
                    </ul>
                </div>
...
function ResultsPanel({ results }) {
    ...
    const { twins, anomalyScore, flags, decision } = results;
    
    return (
        <div className="space-y-6">
            <DecisionBanner decision={decision} anomalyScore={anomalyScore} />
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <TwinsList twins={twins} />
                <AnomalyRadar twins={twins} anomalyScore={anomalyScore} flags={flags} />
            </div>
            
            <CohortCharts twins={twins} decision={decision} />
            
            <ExplanationSection twins={twins} decision={decision} anomalyScore={anomalyScore} />
        </div>
    );
}
